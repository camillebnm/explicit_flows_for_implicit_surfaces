...
def gen_mc_coordinate_grid(N: int, voxel_size: list, t: float = None,
                           device: str = "cuda",
...
def create_mesh(
    ...
    # the middle
    #voxel_origin = [-1, -1, -1]
    voxel_size = [ i / (N - 1) for i in voxel_dim ]
    ...
    def V(x): 
        a=20
        b = torch.exp(-a/2*(x-1)**2)
        c = torch.exp(-a/2*(x)**2) 
        d = torch.exp(-a*(x-1/2)**2) 
        e = torch.sqrt(b**2+c**2+d**2)
        return torch.as_tensor([[b/e], [c/e], [d/e] ])
    """
    ...
    """
    def valval(x): 
        mask = torch.linalg.norm(x[:,:-1], keepdim=True, dim=-1)<np.sqrt(3)
        print(x)
        print(mask.sum())
        return decoder(x)["model_out"]*mask + ~mask*0.2
    """

    def alpha(t): 
        return resize[0] - t*(resize[0]-resize[1]) #+ (torch.sin(t*2*np.pi) + (1-t)*t*torch.cos(t*np.pi/2))/4
    ...
    def rescaled_inr(x):
        ...
        return alpha(t)*decoder(zz)["model_out"]
...
def create_slice(
    ...
    sdf_values = samples[:, sdf_coord]
    sdf_values = sdf_values.reshape(N, N, N).detach().cpu().numpy()
    ...
    fig, ax = plt.subplots()
    val = ax.imshow(np.flip(sdf_values[..., int(N/2)-1, :,:], 0))
    fig.colorbar(val)
    ax.contour(np.flip(sdf_values[..., int(N/2)-1, :, :], 0), levels=[0], colors = "k")
...
def convert_sdf_samples_to_ply(
    ...
    if level < numpy_3d_sdf_tensor.min() or level > numpy_3d_sdf_tensor.max():
        ...
        verts, faces, normals, values = marching_cubes(
            numpy_3d_sdf_tensor, level, spacing=voxel_size
        )
