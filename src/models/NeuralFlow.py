...
class Affine_Coupling_Sir_NF(nn.Module):
    def __init__(self, mask, hidden_dim, depth, L, omega=60): #L = 0 : no pos_embedding
        super(Affine_Coupling_Sir_NF, self).__init__()
    ...
    def forward(self, x,time):
        ...
        y = self.mask[:-1]*x + (1-self.mask[:-1])*(x*torch.exp(s*time) + time*t)        
        logdet = torch.sum((1 - self.mask[:-1])*s*time, -1)
    ...
    def inverse(self, y,time):
        ...
        x = self.mask[:-1]*y + (1-self.mask[:-1])*((y - t*time)*torch.exp(-s*time))
        logdet = torch.sum((1 - self.mask[:-1])*(-s*time), -1)
...
class Affine_Coupling(nn.Module):
    def __init__(self, mask, hidden_dim, L): #L = 0 : no pos_embedding
        ...
        self.act = torch.tanh #torch.nn.ELU(alpha=1.0, inplace=False) #torch.relu #
...
class NeuralFlow(nn.Module):
    ...
    def __init__(self, hidden_dim = 128, L = 0, style = None,archi = ["full", 2, 2]):
        ...
        if style=="siren" : 
            ...
            [Affine_Coupling_Sir_NF(self.masks[i], self.hidden_dim, int(archi[2]),  self.L)
